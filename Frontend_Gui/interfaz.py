...
from controller import middleware , webcam

#-------------------------------------------------------------------------------------------
# 
#                   IMPORTACION DE PAQUETES Y SUBMODULOS DE LA APLICACION
# -------------------------------------------------------------------------------------------
#loader_service_auth=importlib.import_module('.controller.' +'middleware',package='Frontend Gui')
#loader_webcam=importlib.import_module('.controller.' + 'webcam',package=__package__)
...
def verificar_usuario():
    usuario_login,similitud=middleware.login_facial(verificacion_usuario,pantalla2)
...
def registro_facial_interfaz():
    if not middleware.validar_datos(usuario):
        usuario_entrada.delete(0, END)  # Limpiamos los text variables
        Label(pantalla1, text="Ningun campo debe quedar vacio", fg="red", font=("Calibri", 11)).pack()

    else:
        if middleware.registro_facial(usuario):
            usuario_entrada.delete(0, END)  # Limpiamos los text variables
...
def pantalla_principal():
    ...
    Label(text="").pack()  # Creamos el espacio entre el primer boton y el segundo boton
    Button(text="Iniciar detección facial", height="2", width="30", command=webcam.iniciar_webcam).pack()
    pantalla.mainloop()
